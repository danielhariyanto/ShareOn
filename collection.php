<?php
	...
	if (isset($_SESSION['filters']) && !empty($_SESSION['filters'])) {
		$cleanget=$_SESSION['filters'];
			...
			if(isset($cleanget['only-people']) && !isset($_GET['user'])){
		...
		$ps = new triagens\ArangoDb\collections();
		$parray= $ps->getPostsByKey($start,$rules,"time DESC",htmlentities($_GET['collection']));
		foreach($parray as $key){
		    if(isset($key['content'],$key['type'],$key['view_option'])) {
                ...
                $content = (isset($c['processor'])) ? arrange($key['content'], $c['processor']) : $key['content'];
                    echo "<div class='postbox' key='" . $key['_key'] . "'>
